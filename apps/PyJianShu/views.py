...
def articleCount(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("select u.name, u.home_page,count(a2.id) as article_count "
                       ...
                       "as a2 on u.id=a2.user_id group by u.id order by article_count desc, u.js_id",[time_start, time_end])
...
def commentCount(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("select u.name, u.home_page,count(a2.id) as comment_count "
                       ...
                       "as a2 on u.id=a2.user_id group by u.id order by comment_count desc, u.js_id", [time_start, time_end])
...
def likeCount(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("select u.name, u.home_page,count(a2.id) as like_count "
                       ...
                       "as a2 on u.id=a2.user_id group by u.id order by like_count desc, u.js_id", [time_start, time_end])
